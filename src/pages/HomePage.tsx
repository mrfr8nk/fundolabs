...
function ProFooter() {
  const links = {
    ...
    Curriculum: [
      { label: "ZIMSEC O-Level", href: "/curriculum/zimsec-o-level" },
      { label: "ZIMSEC A-Level", href: "/curriculum/zimsec-a-level" },
      { label: "Cambridge IGCSE", href: "/curriculum/cambridge-igcse" },
      { label: "IB Science", href: "/curriculum/ib-science" },
    ],
    Company: [
          ...
          {Object.entries(links).map(([title, items]) => (
            <div key={title}>
              ...
              <ul className="space-y-3">
                {items.map((item) => (
                  <li key={item.label}>
                    <Link to={item.href} className="text-sm text-muted-foreground hover:text-foreground transition flex items-center gap-1.5 group">
