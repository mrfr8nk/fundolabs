...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (!user) return;
    (async () => {
      const [{ count: sessions }, { count: reports }, { count: exams }, { count: saved }, { data: profile }] = await Promise.all([
    ...
    <div className="space-y-8 max-w-6xl">
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-primary/20 to-accent/20 flex items-center justify-center group-hover:glow-sm transition">
                    {exp.subject === "chemistry" ? <FlaskConical className="h-4 w-4 text-primary" /> : <Atom className="h-4 w-4 text-primary" />}
                  </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="glass-card rounded-2xl p-6">
          ...
          <div className="space-y-3">
            {[
              { tip: "In titrations, always rinse your burette with the solution before filling.", type: "Chemistry" },
              { tip: "For Ohm's Law graphs: the gradient of V vs I = resistance. Ammeter must be in series!", type: "Physics" },
              { tip: "When writing observations: say WHAT you see (colour, precipitate, gas), not WHY.", type: "Exam Tip" },
              { tip: "OIL RIG: Oxidation Is Loss of electrons, Reduction Is Gain of electrons.", type: "Chemistry" },
            ].map(({ tip, type }) => (
