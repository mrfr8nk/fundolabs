...
const EXAM_QUESTIONS: Record<string, ExamQuestion[]> = {
  "acid-base-titration": [
    { question: "State the colour change at the endpoint when phenolphthalein is used as the indicator in this titration.", answer: "The solution changes from colourless to permanent pale pink.", marks: 1, type: "observation" },
    ...
    { question: "Explain why you should repeat the titration to obtain concordant titres.", answer: "To reduce random errors / to increase reliability of the result. Concordant titres (within 0.10 cm³) give a more accurate mean titre.", marks: 2, type: "conclusion" },
  ...
  "ohms-law": [
    { question: "Describe what happens to the current when the voltage across the resistor is doubled, assuming the resistance remains constant.", answer: "The current doubles. This is consistent with Ohm's Law: I = V/R. Doubling V doubles I since R is constant.", marks: 2, type: "conclusion" },
    { question: "Calculate the resistance of a conductor if a current of 0.3 A flows when a voltage of 6 V is applied.", answer: "R = V/I = 6/0.3 = 20 Ω", marks: 2, type: "calculation" },
    { question: "What does the gradient of a voltage-current (V-I) graph represent for an ohmic conductor?", answer: "The gradient = V/I = resistance (in ohms, Ω). The graph is a straight line through the origin.", marks: 1, type: "observation" },
    { question: "State why the ammeter must be connected in series and the voltmeter in parallel.", answer: "Ammeter in series: to measure the same current flowing through the circuit. Voltmeter in parallel: to measure the potential difference (voltage) across the component without significantly affecting the current.", marks: 2, type: "method" },
  ],
  "pendulum": [
    { question: "State TWO variables that must be kept constant in this experiment.", answer: "Mass of the bob; amplitude (angle) of oscillation; any two from: material of bob, shape, position of pivot.", marks: 2, type: "method" },
    ...
    { question: "Explain why 20 oscillations are timed rather than just 1.", answer: "To reduce the effect of reaction time (random error) when using a stopwatch. Timing 20 oscillations and dividing by 20 gives a much more accurate value of T.", marks: 2, type: "conclusion" },
  ],
};

const DEFAULT_QUESTIONS: ExamQuestion[] = [
  { question: "State the aim of this experiment.", answer: "The aim is stated in the experiment description above.", marks: 1, type: "method" },
...
function ExamMode() {
  ...
  const questions = selectedExp
    ? (EXAM_QUESTIONS[selectedExp] ?? DEFAULT_QUESTIONS)
    : DEFAULT_QUESTIONS;
  ...
  const startExam = (slug: string) => {
    ...
    const duration = (exp?.duration_minutes ?? 20) * 60;
  ...
  const submitExam = async () => {
    ...
    qs.forEach((q, i) => {
      ...
      earned += partial;
      feedback.push(partial >= q.marks ? `✓ Q${i + 1}: Full marks (${q.marks}/${q.marks})` :
        partial > 0 ? `~ Q${i + 1}: Partial (${partial}/${q.marks}) — ${q.answer}` :
        `✗ Q${i + 1}: 0/${q.marks} — ${q.answer}`);
    });
      ...
      <div className="max-w-4xl space-y-8">
        ...
        <div className="glass-card rounded-2xl p-6 border border-accent/20">
          ...
          <ul className="space-y-2 text-sm text-muted-foreground">
            {[
              "You will be given ZIMSEC-style questions based on the experiment",
              "A countdown timer matches the real exam duration",
              "Type your answers as you would in a real exam",
              "AI grades your answers against model answers and gives feedback",
              "Your score is saved to your profile for tracking progress",
            ].map((item) => (
              <li key={item} className="flex items-start gap-2">
                <CheckCircle2 className="h-4 w-4 text-accent flex-shrink-0 mt-0.5" /> {item}
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="max-w-3xl space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div className={`glass-card rounded-xl px-4 py-2 flex items-center gap-2 font-mono font-bold text-lg ${urgent ? "border-destructive/50 text-destructive" : "text-accent"}`}>
            ...
            {formatTime(timeLeft)}
            {urgent && <span className="text-xs animate-pulse">⚠️</span>}
          </div>
        </div>

        {experiment && (
          <div className="glass-card rounded-xl p-4 text-sm">
            <span className="text-muted-foreground">Experiment: </span>{experiment.description}
          </div>
        )}
          ...
          {questions.map((q, i) => (
            <div key={i} className="glass-card rounded-2xl p-5">
              <div className="flex items-start justify-between gap-3 mb-3">
                <div>
                  <span className="text-xs font-semibold text-primary mr-2">Q{i + 1}</span>
    ...
    return (
      <div className="max-w-3xl space-y-6">
        ...
        <div className="glass-card rounded-2xl p-6 space-y-3">
          ...
          {score.feedback.map((f, i) => (
            <div key={i} className={`text-sm p-3 rounded-xl ${
              f.startsWith("✓") ? "bg-accent/10 text-accent" :
              f.startsWith("~") ? "bg-yellow-500/10 text-yellow-400" :
        ...
        <div className="flex gap-3">
          <Button onClick={() => { setPhase("select"); setScore(null); }} className="flex-1 glass-card border-white/15">
            Try Another Exam
          </Button>
          <Button className="flex-1 bg-gradient-to-r from-primary to-accent text-primary-foreground">
            <Brain className="h-4 w-4 mr-2" /> Review with AI Tutor
          </Button>
        </div>
      </div>
    );
